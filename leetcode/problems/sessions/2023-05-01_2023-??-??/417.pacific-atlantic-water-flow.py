    ...
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def dfs(row, col, visited_set, prev_height):
            ...
            for d_row, d_col in DIRECTIONS:
                dfs(row + d_row, col + d_col, visited_set, heights[row][col])

        for col in range(COLS):
            dfs(0, col, pacific_visited_set, heights[0][col])
            dfs(ROWS - 1, col, atlantic_visited_set, heights[ROWS - 1][col])

        for row in range(ROWS):
            dfs(row, 0, pacific_visited_set, heights[row][0])
            dfs(row, COLS - 1, atlantic_visited_set, heights[row][COLS - 1])
