...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def pacificAtlanticHelper(row, col, visited, prev_height):
            if (
                not 0 <= row <= ROWS - 1 or
                not 0 <= col <= COLS - 1 or
        ...
        for row in range(ROWS):
            pacificAtlanticHelper(
                row, 0, pacific_visited_set, heights[row][0]
            )
            pacificAtlanticHelper(
        ...
        for row in range(ROWS):
            for col in range(COLS):
                if (
                    (row, col) in pacific_visited_set and
