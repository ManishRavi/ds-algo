...
# * Iterative Sorting Solution | O(mn*(log(min(m, n)))) Time | O(mn) Space
# * m -> The number of rows in the mat matrix | n -> The number of columns in the mat matrix
    ...
    def diagonalSort(self, mat: List[List[int]]) -> List[List[int]]:
        ...
        for row in range(ROWS):
            for col in range(1 if row else COLS):
                ...
                diagonal = self.get_diagonal_elements(mat, row, col, ROWS, COLS)
                diagonal.sort()
                # * Put the sorted elements back to their respective positions in the given matrix.
                for i, j, k in zip(
                    range(row, ROWS), range(col, COLS), range(len(diagonal))
                ):
                    mat[i][j] = diagonal[k]
