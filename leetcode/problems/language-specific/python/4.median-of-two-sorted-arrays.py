...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        left, right = 0, nums1_length
        while left <= right:
            partition1 = left + (right - left) // 2
            # * total_length + 1 -> To handle both even and odd lengths
            partition2 = ((total_length + 1) // 2) - partition1

            left1 = nums1[partition1 - 1] if partition1 > 0 else float("-inf")
            right1 = nums1[partition1] if partition1 < nums1_length else float("inf")
            left2 = nums2[partition2 - 1] if partition2 > 0 else float("-inf")
            right2 = nums2[partition2] if partition2 < nums2_length else float("inf")

            if left1 <= right2 and left2 <= right1:
                # * If even length
                if not total_length % 2:
