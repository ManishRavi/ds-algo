...
class MedianFinder:
    ...
    def addNum(self, num: int) -> None:
        ...
        if (
            self.small_max_heap and self.large_min_heap and
            not -self.small_max_heap[0] <= self.large_min_heap[0]
        ):
            heapq.heappush(
                self.large_min_heap,
                -heapq.heappop(self.small_max_heap)
            )
        ...
        if len(self.large_min_heap) > len(self.small_max_heap) + 1:
            heapq.heappush(
                self.small_max_heap,
                -heapq.heappop(self.large_min_heap)
            )
