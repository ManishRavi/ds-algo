...
# * Recursive Bottom-Up Solution | O(n*(sum(nums))) Time | O(n*(sum(nums))) Space
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        # * index and current sum and value is the total ways.
        ...
        def findTargetSumWaysHelper(start_idx, cur_sum):
            ...
            if (start_idx, cur_sum) in dp:
                return dp[(start_idx, cur_sum)]
            ...
            dp[(start_idx, cur_sum)] = add_ways + sub_ways
            return dp[(start_idx, cur_sum)]
