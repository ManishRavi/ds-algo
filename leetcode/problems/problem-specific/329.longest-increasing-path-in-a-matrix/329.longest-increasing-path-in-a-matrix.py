...
class Solution:
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        def longestIncreasingPathHelper(row, col, prev_val):
            if (
                not 0 <= row <= ROWS - 1
                or not 0 <= col <= COLS - 1
            ...
            for d_row, d_col in DIRECTIONS:
                cur_len = max(
                    cur_len,
                    longestIncreasingPathHelper(
                        row + d_row, col + d_col, matrix[row][col]
                    ),
                )
        ...
        for row in range(ROWS):
            for col in range(COLS):
                max_len = max(max_len, longestIncreasingPathHelper(row, col, -1))
