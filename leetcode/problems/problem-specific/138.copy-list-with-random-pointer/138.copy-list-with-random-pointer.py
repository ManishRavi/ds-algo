...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        original_to_copy_mappings = collections.defaultdict(lambda: None)

        # * 1st Pass -> Copy a node and store the mapping of the original to copy in a hashmap.
        cur = head
        while cur:
            original_to_copy_mappings[cur] = Node(cur.val)
            cur = cur.next
        ...
        cur = head
        while cur:
            copy_node = original_to_copy_mappings[cur]
            copy_node.next = original_to_copy_mappings[cur.next]
            copy_node.random = original_to_copy_mappings[cur.random]
            cur = cur.next

        return original_to_copy_mappings[head]
